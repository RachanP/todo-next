...
export const TodoFilters: React.FC<TodoFiltersProps> = ({
  ...
  return (
    <div className="bg-slate-800/80 border border-slate-700/80 rounded-2xl p-4 sm:p-5 space-y-4 shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-9 py-2 bg-slate-900/90 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-500 text-xs sm:text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors"
          />
          {filters.search && (
            <button
        ...
        <div className="flex items-center space-x-2 shrink-0 justify-end">
          ...
          {isFiltered && (
            <button
              ...
              className="px-3 py-2 bg-slate-700/60 hover:bg-slate-700 text-slate-300 rounded-xl text-xs flex items-center space-x-1.5 transition-colors border border-slate-600/60"
            >
      ...
      {/* Bottom Row: Filter Dropdowns & Sorting */}
      ...
      <div className="text-[11px] text-slate-400 flex items-center justify-between pt-1 border-t border-slate-700/50">
        <span>
          แสดงผล <strong className="text-slate-200">{totalFilteredCount}</strong> รายการ
        </span>
        {isFiltered && (
          <span className="text-indigo-400 font-medium">กำลังเปิดใช้งานตัวกรองค้นหา</span>
        )}
      </div>
